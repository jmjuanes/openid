...
class RouteLogin(webapp2.RequestHandler):
    def get(self):
        return template(self, 'login.html')
    ...
    def post(self):
        ...
        if user_email == '' or user_pwd == '':
            ...
            return template(self, 'error.html', error='Invalid user request')
        ...
        if config.captcha_enabled is True:
            ...
            if captcha.verify(captcha_value, config.captcha_secret, config.captcha_url) is False:
                self.response.status_int = 400
                return template(self, 'login.html',  error='Invalid captcha')

        # Get the user from the database
        user = db_user.get_user(user_email)
        if user is None:
            self.response.status_int = 400
            return template(self, 'login.html', error='Email not found')

        # Check if user is active
        if user.active is False:
            self.response.status_int = 401
            return template(self, 'login.html', error='User is not active')
        ...
        if pbkdf2_sha256.verify(user_pwd, user.pwd) is True:
            ...
            self.response.status_int = 400
            return template(self, 'login.html', error='Invalid email or password')
...
class RouteRegister(webapp2.RequestHandler):
    def get(self):
        ...
        return template(self, 'register.html', error='')
    ...
    def post(self):
        ...
        if user.name is '' or user.email is '' or user.pwd is '':
            self.response.status_int = 400
            return template(self, 'register.html', error='You must fill all fields')
        ...
        if config.captcha_enabled is True:
            ...
            if captcha.verify(captcha_value, config.captcha_secret, config.captcha_url) is False:
                self.response.status_int = 400
                return template(self, 'register.html',  error='Invalid captcha')

        # Check if the email is registered
        if db_user.exists_user(user.email):
            self.response.status_int = 400
            return template(self, 'register.html', error='The email has already registered')

        # Register the user
        try:
            user_key = user.put()
            # continue_url = request.args.get('continue', '')
            return template(self, 'register-completed.html')
        except:
            return template(self, 'register.html', error='Error creating the user. Contact with the administrator')
...
class RouteAuthorize(webapp2.RequestHandler):
    def get(self):
        ...
        if application_id == '':
            self.response.status_int = 400
            return template(self, 'error.html', error='No application id provided')

        # Get the application information and check if exists
        application = db_application.get_application(application_id)
        if application is not None:
            return template(self, 'authorize.html',
                            app_id=application_id,
                            app_signup=application_signup,
                            app_name=application.name)
        else:
            self.response.status_int = 404
            return template(self, 'error.html', error='Invalid application')
    ...
    def post(self):
        ...
        if application_id == '':
            self.response.status_int = 400
            return template(self, 'error.html', error='Invalid application request')
        ...
        if application is None:
            self.response.status_int = 400
            return template(self, 'error.html', error='Application not found')
